...
- Source digest: `50d0780efc637124`
...
| tokei | ok | tokei --output json . --exclude 'docs/architecture/code-map*' | Code lines: 10176; top languages: JSON=5368, YAML=2862, JavaScript=884, TSX=520, CSS=343 |  |
| scc | ok | scc --format json --not-match '^docs/architecture/code\-map.*$' . | Complexity: 171; top languages: JSON=5368, CSV=976, JavaScript=884, TypeScript=626, CSS=343 |  |
| cloc | ok | cloc --json --quiet --vcs git --exclude-list-file build/code-cartographer/cloc-exclude.txt | Files: 38; code lines: 11448; top languages: JSON=5368, YAML=2862, CSV=976, JavaScript=883, TypeScript=626 |  |
...
| apps/web/scripts/catalog/sync-support.mjs | source | auth | JavaScript | 275 | array, atomicWriteJson, defaultExternalPaths, fetchAccessToken, fetchLiveMedia, flattenCategories, loadDatabaseFallbacks, parseOptionalLiveThumbnail | better-sqlite3, fast-xml-parser, node:fs, node:path |
| apps/web/scripts/catalog/sync-support.test.mjs | source | auth | JavaScript | 107 |  | better-sqlite3, node:assert, node:fs, node:os, node:path, node:test |
